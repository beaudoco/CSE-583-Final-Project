...
pca_train_images = np.load(SAVE_PATH + "pca_train_images.npy")
pca_test_images = np.load(SAVE_PATH + "pca_test_images.npy")

#distribute between test and validate
x_val = pca_train_images[-2000:,:,:,:]
y_val = y_train[-2000:]
x_train = pca_train_images[:-2000,:,:,:]
...
#load saved model
model.load_weights("keras_models/pca_model")

#begin creating confusion for training
train_prediction = model.predict(x_train)
val_prediction = model.predict(x_val)
test_prediction = model.predict(pca_test_images)
...
ax_train.set_title('CNN + PCA Training Confusion Matrix\n\n');
ax_train.set_xlabel('\nPredicted Values')
...
ax_val.set_title('CNN + PCA Validation Confusion Matrix\n\n');
ax_val.set_xlabel('\nPredicted Values')
...
ax_test.set_title('CNN + PCA Test Confusion Matrix\n\n');
ax_test.set_xlabel('\nPredicted Values')
